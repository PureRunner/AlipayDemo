...
#import "ClientDownloadManager.h"
#import <AFNetworking/AFNetworking.h>
#import <objc/runtime.h>
#import "NSString+ClientDownload.h"

NSString *const ClientDownloadSessionIdentifier = @"com.BSL.BSLDownloadSessionIdentifier";

NSString *const ClientDownloadURLStringKey = @"com.BSL.BSLDownloadManager.URLStringKey";
NSString *const ClientDownloadFileNameKey = @"com.BSL.BSLDownloadManager.FileNameKey";

static void * progressBlockKey;


@interface ClientDownloadManager ()
...
@end
@implementation ClientDownloadManager
...
- (instancetype)initWithCachePath:(NSString *)cachePath {
    ...
    if (self) {
        if (!cachePath) {
            ...
            self.cachePath = [cachePath stringByAppendingPathComponent:@"com.BSL.BSLDownloadManager"];
        }else {
...
- (NSURLSessionDownloadTask *)downloadWithConstructBlock:(ClientDownloadConstructBlock)constructBlock progressBlock:(ClientDownloadProgressBlock)progressBlock completeBlock:(ClientDownloadCompleteBlock)completeBlock {
    ...
    NSString *URLString = arguments[ClientDownloadURLStringKey];
    NSAssert(URLString, @"must have url");
    NSString *fileName = arguments[ClientDownloadFileNameKey];
    if (!fileName) {
...
- (AFURLSessionManager *)sessionManager {
    if (!_sessionManager) {
        NSURLSessionConfiguration *sessionConfiguration = [NSURLSessionConfiguration backgroundSessionConfigurationWithIdentifier:ClientDownloadSessionIdentifier];
        ...
        [_sessionManager setDownloadTaskDidWriteDataBlock:^(NSURLSession * _Nonnull session, NSURLSessionDownloadTask * _Nonnull downloadTask, int64_t bytesWritten, int64_t totalBytesWritten, int64_t totalBytesExpectedToWrite) {
            ClientDownloadProgressBlock progressBlock = objc_getAssociatedObject(downloadTask, &progressBlockKey);
            if (progressBlock) {
